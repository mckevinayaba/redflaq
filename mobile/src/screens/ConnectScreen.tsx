...
const MATCHES = [
  { name: 'Nomsa K.',  age: 29, city: 'Johannesburg', score: 94, bio: "I've been on my healing journey for two years. Looking for women who get it — who don't need everything explained.", tags: ['Safety-conscious', 'Empathetic', 'Ambitious'], initial: 'N', color: '#6C35DE' },
  { name: 'Ayanda M.', age: 34, city: 'Cape Town',    score: 88, bio: 'Mother, writer, advocate. I believe in chosen family. Looking for genuine connection with women building intentional, safe lives.', tags: ['Family-oriented', 'Creative', 'Strong values'], initial: 'A', color: '#C0392B' },
  { name: 'Thandi N.', age: 26, city: 'Durban',       score: 81, bio: 'I run a community safety initiative. Avid reader. Looking for friendships built on honesty, not performance.', tags: ['Activist', 'Outdoors', 'Direct'], initial: 'T', color: '#27AE60' },
  { name: 'Lerato P.', age: 31, city: 'Pretoria',     score: 76, bio: 'Civil engineer. Cautious by experience, optimistic by choice. Love Saturday markets and long conversations.', tags: ['Pragmatic', 'Warm', 'Grounded'], initial: 'L', color: '#E67E22' },
];
...
function isVerified(checks: ReturnType<typeof useSavedChecks>['checks']): boolean {
  ...
  return checks.some(c => Date.now() - new Date(c.savedAt).getTime() < ms90);
}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(8,8,15,0.92)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div style={{ background: '#111118', borderTop: '1px solid rgba(108,53,222,0.3)', borderRadius: '16px 16px 0 0', padding: '28px 24px 36px', maxWidth: 480, width: '100%' }}>
        ...
        <button onClick={onAccept} style={{ width: '100%', padding: '16px', background: '#6C35DE', border: 'none', borderRadius: 6, cursor: 'pointer', ...inter, fontSize: 15, fontWeight: 800, color: '#ffffff', marginBottom: 12 }}>
          I Understand — Start Quiz
        </button>
        <button onClick={onDismiss} style={{ width: '100%', padding: '12px', background: 'none', border: 'none', cursor: 'pointer', ...inter, fontSize: 14, color: '#8b8b91' }}>
          Not now
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ background: '#08080f', height: '100%', overflowY: 'auto', paddingBottom: 90 }}>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        <div style={{ background: 'rgba(217,119,6,0.08)', border: '1px solid rgba(217,119,6,0.25)', borderRadius: 6, padding: '12px 14px', marginBottom: 24, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          <span style={{ fontSize: 14, flexShrink: 0 }}>⚠️</span>
          <p style={{ ...inter, fontSize: 12, color: '#D97706', lineHeight: 1.55 }}><strong>Demo results.</strong> Mock profiles for demonstration. Real SafeMatch with verified profiles is coming soon. Always run a RedFlaq check before meeting anyone.</p>
        </div>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
                      <p style={{ ...inter, fontSize: 15, fontWeight: 800, color: '#ffffff', letterSpacing: '-0.01em' }}>{m.name}, {m.age}</p>
                      <span style={{ ...mono, fontSize: 10, fontWeight: 700, color: m.color, background: m.color + '1a', border: `1px solid ${m.color}33`, borderRadius: 3, padding: '2px 7px', letterSpacing: '0.06em' }}>{m.score}%</span>
                    </div>
                    <p style={{ ...mono, fontSize: 9, color: '#8b8b91', letterSpacing: '0.06em' }}>{m.city}</p>
                  </div>
                </div>
                <p style={{ ...inter, fontSize: 13, color: '#8b8b91', lineHeight: 1.6, marginBottom: 12 }}>{m.bio}</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {m.tags.map(tag => (
                    <span key={tag} style={{ ...mono, fontSize: 8, color: '#8b8b91', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 3, padding: '3px 7px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{tag}</span>
                  ))}
                </div>
              </div>
            </div>
                ...
                <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      <span style={{ ...mono, fontSize: 8, color: clr, background: clr + '18', border: `1px solid ${clr}33`, borderRadius: 3, padding: '2px 6px', letterSpacing: '0.06em', flexShrink: 0 }}>{g.matchScore}%</span>
                    </div>
                    <p style={{ ...mono, fontSize: 8, color: '#8b8b91', letterSpacing: '0.06em' }}>{g.memberCount} members · {g.province}</p>
                  </div>
                  {g.verificationRequired && (
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
                      <rect x="3" y="11" width="18" height="11" rx="2" stroke="#6C35DE" strokeWidth="1.8"/>
                      <path d="M7 11V7a5 5 0 0110 0v4" stroke="#6C35DE" strokeWidth="1.8" strokeLinecap="round"/>
                    </svg>
                  )}
                </div>
              </div>
        ...
        <button onClick={onBrowseGroups} style={{ width: '100%', padding: '14px', background: 'rgba(108,53,222,0.12)', border: '1px solid rgba(108,53,222,0.3)', borderRadius: 6, cursor: 'pointer', ...inter, fontSize: 14, fontWeight: 700, color: '#6C35DE', marginBottom: 16 }}>
          Browse All Groups →
        </button>

        <div style={{ background: '#111118', border: '1px solid rgba(108,53,222,0.2)', borderRadius: 8, padding: '20px 18px', textAlign: 'center', marginBottom: 20 }}>
          <p style={{ ...mono, fontSize: 9, color: '#6C35DE', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>Direct Messaging — Coming Soon</p>
          <p style={{ ...inter, fontSize: 13, color: '#8b8b91', lineHeight: 1.6, maxWidth: 260, margin: '0 auto' }}>Secure, in-app messaging with your matches — all safety-checked — is in development.</p>
        </div>

        <button onClick={onRetake} style={{ width: '100%', padding: '13px', background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, ...inter, fontSize: 13, fontWeight: 600, color: '#8b8b91', cursor: 'pointer' }}>
          Retake Quiz
        </button>
      </div>
    </div>
...
// ── Home view ──────────────────────────────────────────────────
function HomeView({ quizStarted, quizComplete, progress, onStartQuiz, onResumeQuiz, onViewResults, onBrowseGroups }: {
  quizStarted: boolean; quizComplete: boolean; progress: number;
  onStartQuiz: () => void; onResumeQuiz: () => void; onViewResults: () => void; onBrowseGroups: () => void;
}) {
  return (
    <div style={{ background: '#08080f', height: '100%', overflowY: 'auto', paddingBottom: 90 }}>
      <div style={{ padding: '56px 20px 20px' }}>
        ...
        <div style={{ background: '#111118', border: quizComplete ? '1px solid rgba(39,174,96,0.3)' : '1px solid rgba(108,53,222,0.3)', borderRadius: 10, overflow: 'hidden', marginBottom: 12 }}>
          ...
          <div style={{ padding: '20px 18px' }}>
            <p style={{ ...mono, fontSize: 8, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10, color: quizComplete ? '#27AE60' : '#6C35DE' }}>
            ...
            <h2 style={{ ...inter, fontSize: 18, fontWeight: 800, color: '#ffffff', letterSpacing: '-0.015em', marginBottom: 8, lineHeight: 1.3 }}>
              {quizComplete ? 'Your profile is ready' : "Find people who understand what you've been through"}
            </h2>
            <p style={{ ...inter, fontSize: 13, color: '#8b8b91', lineHeight: 1.55, marginBottom: 18 }}>
              {quizComplete ? '4 compatible connections found based on your answers.' : '40 questions across 5 sections. Friendship-first. Stored locally.'}
            </p>
        ...
        <button onClick={onBrowseGroups} style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginBottom: 24 }}>
          <div style={{ background: '#111118', border: '1px solid rgba(108,53,222,0.2)', borderRadius: 10, padding: '18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
              <path d="M9 18l6-6-6-6" stroke="#6C35DE" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
        </button>
...
function GroupsView({ quizComplete, joinedGroups, onSelectGroup, onCreateGroup, onBack }: {
  quizComplete: boolean; joinedGroups: string[];
  onSelectGroup: (g: Group) => void; onCreateGroup: () => void; onBack: () => void;
}) {
  ...
  return (
    <div style={{ background: '#08080f', height: '100%', overflowY: 'auto', paddingBottom: 90 }}>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        ...
        <h1 style={{ ...inter, fontSize: 22, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.02em', marginBottom: 6 }}>
          Find Your{' '}<span style={{ ...playfair, fontStyle: 'italic', color: '#6C35DE' }}>Circle</span>
        </h1>
        <p style={{ ...inter, fontSize: 13, color: '#8b8b91', lineHeight: 1.6, marginBottom: 16 }}>
          {quizComplete ? 'Groups matched to your SafeMatch profile, highest first.' : 'Private groups for women navigating healing, legal rights, and more.'}
        </p>
        ...
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4, marginBottom: 20, scrollbarWidth: 'none' }}>
          {GROUP_CATEGORIES.map(cat => {
            ...
            return (
              <button key={cat.id} onClick={() => setActiveCategory(cat.id)} style={{ flexShrink: 0, padding: '7px 14px', background: active ? '#6C35DE' : '#111118', border: active ? '1px solid #6C35DE' : '1px solid rgba(255,255,255,0.08)', borderRadius: 20, cursor: 'pointer', ...mono, fontSize: 9, color: active ? '#ffffff' : '#8b8b91', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>
                {cat.label}
              </button>
            );
          })}
          ...
          {sorted.map(g => {
            ...
            return (
              <button key={g.id} onClick={() => onSelectGroup(g)} style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                <div style={{ background: '#111118', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, overflow: 'hidden', transition: 'border-color 0.15s' }}>
                  ...
                  <div style={{ padding: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                          ...
                          {quizComplete && (
                            <span style={{ ...mono, fontSize: 8, color: clr, background: clr + '18', border: `1px solid ${clr}33`, borderRadius: 3, padding: '2px 6px', letterSpacing: '0.06em', flexShrink: 0 }}>{g.matchScore}% match</span>
                          )}
                          {joined && (
                            <span style={{ ...mono, fontSize: 8, color: '#27AE60', background: 'rgba(39,174,96,0.12)', border: '1px solid rgba(39,174,96,0.3)', borderRadius: 3, padding: '2px 6px', letterSpacing: '0.06em', flexShrink: 0 }}>Requested</span>
                          )}
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <span style={{ ...mono, fontSize: 8, color: '#8b8b91', letterSpacing: '0.04em' }}>{g.memberCount} members</span>
                          <span style={{ ...mono, fontSize: 8, color: 'rgba(255,255,255,0.2)' }}>·</span>
                          <span style={{ ...mono, fontSize: 8, color: '#8b8b91', letterSpacing: '0.04em' }}>{g.province}</span>
                        </div>
                      </div>
                      {g.verificationRequired && (
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                      {g.tags.map(tag => (
                        <span key={tag} style={{ ...mono, fontSize: 7, color: '#8b8b91', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 3, padding: '3px 7px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
        ...
        <div style={{ background: '#111118', border: '1px solid rgba(108,53,222,0.2)', borderRadius: 8, padding: '18px', textAlign: 'center', marginBottom: 24 }}>
          ...
          <button onClick={onCreateGroup} style={{ padding: '10px 20px', background: 'rgba(108,53,222,0.12)', border: '1px solid rgba(108,53,222,0.3)', borderRadius: 4, cursor: 'pointer', ...inter, fontSize: 13, fontWeight: 700, color: '#6C35DE' }}>
            Create a Group →
          </button>
        </div>
      </div>
...
// ── Group detail view ──────────────────────────────────────────
function GroupDetailView({ group, joined, onBack, onRequestJoin }: { group: Group; joined: boolean; onBack: () => void; onRequestJoin: () => void }) {
  const clr = CATEGORY_COLOR[group.category];
  const verifiedCount = group.members.filter(m => m.verified).length;
  ...
  return (
    <div style={{ background: '#08080f', height: '100%', overflowY: 'auto', paddingBottom: 100 }}>
      ...
      {/* Hero */}
      <div style={{ padding: '0 20px 24px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ height: 4, background: clr, margin: '0 -20px 20px', marginTop: 0 }} />
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          {group.verificationRequired && (
            <span style={{ ...mono, fontSize: 8, color: '#6C35DE', background: 'rgba(108,53,222,0.1)', border: '1px solid rgba(108,53,222,0.3)', borderRadius: 3, padding: '3px 8px', letterSpacing: '0.08em' }}>Verification Required</span>
          )}
        </div>
        <h1 style={{ ...inter, fontSize: 22, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.02em', marginBottom: 8 }}>{group.name}</h1>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <span style={{ ...mono, fontSize: 9, color: '#8b8b91', letterSpacing: '0.04em' }}>{group.memberCount} members</span>
          <span style={{ ...mono, fontSize: 9, color: 'rgba(255,255,255,0.2)' }}>·</span>
          <span style={{ ...mono, fontSize: 9, color: '#8b8b91', letterSpacing: '0.04em' }}>{group.province}</span>
          <span style={{ ...mono, fontSize: 9, color: 'rgba(255,255,255,0.2)' }}>·</span>
          <span style={{ ...mono, fontSize: 9, color: '#27AE60', letterSpacing: '0.04em' }}>{verifiedCount} verified</span>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
          {group.tags.map(tag => (
            <span key={tag} style={{ ...mono, fontSize: 7, color: '#8b8b91', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 3, padding: '3px 7px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{tag}</span>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 28 }}>
          {group.about.map((para, i) => (
            <p key={i} style={{ ...inter, fontSize: 14, color: '#d1d1d6', lineHeight: 1.7 }}>{para}</p>
          ))}
        </div>

        {/* Chat status */}
        <div style={{ background: '#111118', border: '1px solid rgba(108,53,222,0.2)', borderRadius: 8, padding: '14px 16px', marginBottom: 28, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          ...
          <p style={{ ...inter, fontSize: 13, color: '#8b8b91', lineHeight: 1.55 }}>
            <span style={{ color: '#6C35DE', fontWeight: 700 }}>Chat opens when all members are verified.</span>{' '}Once your request is approved, chat becomes available when every participant in a thread has a valid RedFlaq check.
          </p>
        </div>

        {/* Members */}
        <p style={{ ...mono, fontSize: 9, color: '#8b8b91', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 14 }}>
          Members ({group.members.length} shown)
        </p>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3, flexWrap: 'wrap' }}>
                    ...
                    {m.role !== 'member' && (
                      <span style={{ ...mono, fontSize: 7, color: '#6C35DE', background: 'rgba(108,53,222,0.1)', border: '1px solid rgba(108,53,222,0.25)', borderRadius: 3, padding: '2px 6px', letterSpacing: '0.06em', textTransform: 'capitalize' }}>{m.role}</span>
                    )}
                  </div>
                  <p style={{ ...mono, fontSize: 8, color: '#8b8b91', letterSpacing: '0.04em', marginBottom: 4 }}>{m.province}</p>
                  <p style={{ ...inter, fontSize: 12, color: '#8b8b91', lineHeight: 1.55 }}>{m.bio}</p>
                </div>
              </div>
            </div>
          ))}
          <p style={{ ...mono, fontSize: 8, color: 'rgba(255,255,255,0.2)', letterSpacing: '0.06em', paddingTop: 12 }}>
            + {group.memberCount - group.members.length} more members not shown
          </p>
        </div>
      </div>
      ...
      <div style={{ position: 'fixed', bottom: 72, left: 0, right: 0, maxWidth: 480, margin: '0 auto', padding: '12px 20px', background: '#08080f', borderTop: '1px solid rgba(255,255,255,0.06)', zIndex: 20 }}>
        {joined ? (
          <div style={{ background: 'rgba(39,174,96,0.1)', border: '1px solid rgba(39,174,96,0.3)', borderRadius: 6, padding: '14px', textAlign: 'center' }}>
            <p style={{ ...inter, fontSize: 14, fontWeight: 700, color: '#27AE60' }}>✓ Request Sent — Pending Approval</p>
            <p style={{ ...mono, fontSize: 8, color: '#8b8b91', marginTop: 4, letterSpacing: '0.06em' }}>You'll be notified when your request is reviewed</p>
          </div>
        ) : (
...
// ── Verification gate ──────────────────────────────────────────
function VerificationGateView({ group, checks, onGoCheck, onBack }: {
  group: Group;
  checks: ReturnType<typeof useSavedChecks>['checks'];
  onGoCheck?: () => void;
  onBack: () => void;
}) {
  const latestCheck = checks.length > 0 ? checks.reduce((a, b) => new Date(a.savedAt) > new Date(b.savedAt) ? a : b) : null;
  const days = latestCheck ? daysSince(latestCheck.savedAt) : null;
  ...
  return (
    <div style={{ background: '#08080f', height: '100%', overflowY: 'auto', paddingBottom: 90 }}>
      ...
      <div style={{ padding: '40px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
        ...
        <h2 style={{ ...inter, fontSize: 22, fontWeight: 900, color: '#ffffff', letterSpacing: '-0.02em', marginBottom: 12, maxWidth: 300 }}>
          This group requires a recent safety check
        </h2>
        <p style={{ ...inter, fontSize: 14, color: '#8b8b91', lineHeight: 1.7, marginBottom: 28, maxWidth: 300 }}>
          <strong style={{ color: group.name ? '#d1d1d6' : undefined }}>{group.name}</strong> requires all members to have completed a RedFlaq check within the last 90 days.
        </p>

        {/* Current status */}
        <div style={{ width: '100%', maxWidth: 320, background: '#111118', border: `1px solid ${latestCheck && days !== null && days < 90 ? 'rgba(39,174,96,0.3)' : 'rgba(192,57,43,0.3)'}`, borderRadius: 8, padding: '16px', marginBottom: 28 }}>
          <p style={{ ...mono, fontSize: 9, color: '#8b8b91', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Your Check Status</p>
          {latestCheck && days !== null ? (
            <div>
              <p style={{ ...inter, fontSize: 14, fontWeight: 700, color: days < 90 ? '#27AE60' : '#C0392B', marginBottom: 4 }}>
                {days < 90 ? `✓ Check valid (${days} days ago)` : `✗ Check expired (${days} days ago)`}
              </p>
              <p style={{ ...inter, fontSize: 13, color: '#8b8b91' }}>Last check: {latestCheck.name}</p>
            </div>
          ) : (
        ...
        <div style={{ width: '100%', maxWidth: 320, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button onClick={onGoCheck} style={{ width: '100%', padding: '15px', background: '#6C35DE', border: 'none', borderRadius: 6, cursor: 'pointer', ...inter, fontSize: 15, fontWeight: 800, color: '#ffffff' }}>
            Run a Safety Check — R99
          </button>
          <button onClick={onBack} style={{ padding: '13px', background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, cursor: 'pointer', ...inter, fontSize: 13, fontWeight: 600, color: '#8b8b91' }}>
            Go Back
          </button>
        </div>

        <p style={{ ...mono, fontSize: 8, color: 'rgba(255,255,255,0.2)', letterSpacing: '0.06em', marginTop: 24, maxWidth: 280, lineHeight: 1.6 }}>
          Verification is used only to confirm you've run a check. Your check results are never shared with any group.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ background: '#08080f', height: '100%', overflowY: 'auto', paddingBottom: 90 }}>
      ...
      <div style={{ padding: '24px 20px 0' }}>
        ...
        <p style={{ ...inter, fontSize: 13, color: '#8b8b91', lineHeight: 1.6, marginBottom: 24 }}>
          Before your request is submitted, please review what information will be shared with this group.
        </p>
        ...
        <div style={{ background: '#111118', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '16px', marginBottom: 12 }}>
          ...
          {[
            'Your display name (first name and initial only)',
            'Your province',
            'Your SafeMatch category interests (e.g. "Healing", "Legal")',
            'That you have a RedFlaq account',
          ].map((item, i) => (
        ...
        <div style={{ background: '#111118', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '16px', marginBottom: 20 }}>
          ...
          {[
            'Your ID number or any check results',
            'Your journal entries or documented incidents',
            'Your full name, address, or contact details',
            'Your quiz answers or compatibility scores',
          ].map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: i < 3 ? 10 : 0 }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0, marginTop: 2 }}>
                <path d="M18 6L6 18M6 6l12 12" stroke="#C0392B" strokeWidth="2" strokeLinecap="round"/>
              </svg>
              <p style={{ ...inter, fontSize: 13, color: '#d1d1d6', lineHeight: 1.5 }}>{item}</p>
            </div>
          ))}
        ...
        <div style={{ background: 'rgba(108,53,222,0.06)', border: '1px solid rgba(108,53,222,0.2)', borderRadius: 8, padding: '14px 16px', marginBottom: 24 }}>
          ...
          <p style={{ ...inter, fontSize: 12, color: '#8b8b91', lineHeight: 1.6 }}>
            This processing is conducted under <strong style={{ color: '#d1d1d6' }}>POPIA Section 11(1)(a)</strong> (consent). You may withdraw from the group at any time, and your join request data will be deleted within 30 days of withdrawal. RedFlaq acts as responsible party.
          </p>
        </div>
        ...
        <button onClick={() => setAccepted(a => !a)} style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', padding: '0 0 24px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <p style={{ ...inter, fontSize: 13, color: '#d1d1d6', lineHeight: 1.6 }}>
            I have read and understand what information will be shared. I consent to this processing under POPIA and want to request to join <strong>{group.name}</strong>.
          </p>
        </button>
        ...
        <button onClick={onBack} style={{ width: '100%', padding: '13px', background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, cursor: 'pointer', ...inter, fontSize: 13, color: '#8b8b91' }}>
          Cancel
        </button>
      </div>
    </div>
...
function CreateGroupView({ onBack }: { onBack: () => void }) {
  ...
  const labelStyle: React.CSSProperties = { ...mono, fontSize: 9, color: '#8b8b91', letterSpacing: '0.12em', textTransform: 'uppercase', display: 'block', marginBottom: 8 };
    ...
    return (
      <div style={{ background: '#08080f', height: '100%', overflowY: 'auto', paddingBottom: 90 }}>
        ...
        <div style={{ padding: '40px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          ...
          <p style={{ ...inter, fontSize: 14, color: '#8b8b91', lineHeight: 1.7, marginBottom: 32, maxWidth: 300 }}>
            Your group has been submitted for review. We'll notify you when it goes live — usually within 48 hours.
          </p>
          <button onClick={onBack} style={{ padding: '14px 32px', background: '#6C35DE', border: 'none', borderRadius: 6, cursor: 'pointer', ...inter, fontSize: 14, fontWeight: 800, color: '#ffffff' }}>
            Back to Groups
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '24px 20px 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            ...
            <input value={form.name} onChange={set('name')} maxLength={60} placeholder="e.g. Healing Circle East London" style={inputStyle} />
          </div>

          <div>
            <label style={labelStyle}>Category</label>
            <select value={form.category} onChange={set('category')} style={{ ...inputStyle, appearance: 'none' }}>
              <option value="">Select a category</option>
              {GROUP_CATEGORIES.filter(c => c.id !== 'all').map(c => (
          ...
          <div>
            ...
            <select value={form.province} onChange={set('province')} style={{ ...inputStyle, appearance: 'none' }}>
          ...
          <div>
            ...
            {form.description.length > 0 && form.description.length < 20 && (
              <p style={{ ...mono, fontSize: 8, color: '#C0392B', marginTop: 6, letterSpacing: '0.04em' }}>Minimum 20 characters</p>
            )}
          </div>

          <div>
            <label style={labelStyle}>Group Rules <span style={{ color: 'rgba(255,255,255,0.25)' }}>Optional</span></label>
            <textarea value={form.rules} onChange={set('rules')} maxLength={400} rows={4} placeholder="e.g. No victim-blaming. Confidentiality required. Respect all experiences." style={{ ...inputStyle, resize: 'none', lineHeight: 1.6 }} />
          </div>

          <div style={{ background: 'rgba(108,53,222,0.06)', border: '1px solid rgba(108,53,222,0.2)', borderRadius: 8, padding: '14px 16px' }}>
            <p style={{ ...inter, fontSize: 12, color: '#8b8b91', lineHeight: 1.6 }}>
              By submitting, you agree to moderate this group in line with RedFlaq community guidelines. Verification-required groups must have a founder with a valid check.
            </p>
          </div>

          <button onClick={() => setSubmitted(true)} disabled={!canSubmit} style={{ width: '100%', padding: '15px', background: canSubmit ? '#6C35DE' : 'rgba(255,255,255,0.06)', border: 'none', borderRadius: 6, cursor: canSubmit ? 'pointer' : 'not-allowed', ...inter, fontSize: 15, fontWeight: 800, color: canSubmit ? '#ffffff' : '#8b8b91', transition: 'all 0.2s', marginBottom: 8 }}>
            Submit for Review
          </button>
        </div>
      </div>
...
// ── Main screen ────────────────────────────────────────────────
type ConnectView = 'home' | 'quiz' | 'results' | 'groups' | 'group-detail' | 'verification-gate' | 'join-consent' | 'create-group';
...
export default function ConnectScreen({ onGoCheck }: { onGoCheck?: () => void }) {
  ...
  const goToGroups = (from: 'home' | 'results') => {
    setGroupsFrom(from);
    setView('groups');
  };
  ...
  const handleJoinRequest = () => {
    ...
    if (selectedGroup.verificationRequired && !verified) {
      setView('verification-gate');
    } else {
      setView('join-consent');
    }
  };
  ...
  const handleRetake = () => { reset(); setView('home'); };

  if (view === 'quiz') {
    return <QuizView onComplete={() => setView('results')} onBack={() => setView('home')} />;
  }

  if (view === 'results') {
    return <ResultsView onRetake={handleRetake} onBack={() => setView('home')} onBrowseGroups={() => goToGroups('results')} />;
  }
  ...
  if (view === 'groups') {
    return (
      <GroupsView
        ...
        onBack={() => setView(groupsFrom === 'results' ? 'results' : 'home')}
      />
    );
  }
  ...
  if (view === 'group-detail' && selectedGroup) {
    return (
      <GroupDetailView
        group={selectedGroup}
        joined={joinedGroups.includes(selectedGroup.id)}
  ...
  if (view === 'verification-gate' && selectedGroup) {
    return (
  ...
  if (view === 'join-consent' && selectedGroup) {
    return (
      <JoinConsentView
        group={selectedGroup}
  ...
  if (view === 'create-group') {
    return <CreateGroupView onBack={() => setView('groups')} />;
  }
  ...
  return (
    <>
      <HomeView
        ...
        quizComplete={quizComplete}
        progress={progress}
        onStartQuiz={() => setShowOnboarding(true)}
        onResumeQuiz={() => setView('quiz')}
        onViewResults={() => setView('results')}
        onBrowseGroups={() => goToGroups('home')}
      />
      {showOnboarding && (
        <OnboardingModal
          onAccept={() => { setShowOnboarding(false); start(); setView('quiz'); }}
