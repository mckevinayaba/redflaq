...
const FinalCTASection = () => {
  ...
  return (
    <section 
      ...
      style={{
        background: "linear-gradient(180deg, #F9CFCF 0%, #FADADA 100%)"
      }}
    >
      {/* Floating Particles */}
      {[...Array(15)].map((_, i) => (
      ...
      <div className="max-w-[1000px] mx-auto text-center relative z-10">
        ...
        <div className={`mb-12 ${isVisible ? 'animate-slide-up' : 'opacity-0'}`}>
          <h2 className="font-heading font-black text-4xl md:text-5xl lg:text-[56px] leading-[1.1] mb-6" style={{ color: '#A72828' }}>
        ...
        <div className={`mb-10 ${isVisible ? 'animate-fade-in delay-200' : 'opacity-0'}`}>
          <p className="font-body text-xl md:text-2xl leading-relaxed max-w-[700px] mx-auto" style={{ color: '#C43535' }}>
            15,247 South African women have already checked their date's criminal record.
            <br />
            <strong style={{ color: '#A72828' }}>You're 60 seconds away from peace of mind.</strong>
          </p>
        </div>
        ...
        <div className={`mb-12 ${isVisible ? 'animate-fade-in-scale delay-400' : 'opacity-0'}`}>
          <Button 
            ...
            style={{ 
              color: '#D14444',
              boxShadow: '0 20px 60px rgba(167,40,40,0.3)'
            }}
          >
        ...
        <div className={`grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-12 ${isVisible ? 'animate-fade-in delay-600' : 'opacity-0'}`}>
          {guarantees.map((item, index) => (
            <div
              ...
              style={{ 
                boxShadow: '0 10px 30px rgba(167,40,40,0.15)',
                border: '2px solid #F9CFCF'
              }}
            >
              <div 
                className="w-14 h-14 rounded-full flex items-center justify-center"
                style={{ backgroundColor: '#F9CFCF' }}
              >
                <item.icon className="w-7 h-7" style={{ color: '#D14444' }} />
              </div>
              <p className="font-body font-bold text-sm md:text-base text-center" style={{ color: '#A72828' }}>
                {item.text}
              </p>
            </div>
          ))}
        ...
        <div className={`bg-white rounded-2xl p-8 md:p-10 max-w-[700px] mx-auto ${isVisible ? 'animate-fade-in delay-800' : 'opacity-0'}`}
          style={{ 
            boxShadow: '0 20px 60px rgba(167,40,40,0.25)',
            border: '3px solid #D14444'
          }}
        >
          <p className="font-heading font-bold text-2xl md:text-3xl mb-4" style={{ color: '#D14444' }}>
            ⚠️ Don't Wait Until It's Too Late
          </p>
          <p className="font-body text-lg leading-relaxed" style={{ color: '#C43535' }}>
        ...
        <div className={`mt-16 ${isVisible ? 'animate-fade-in delay-1000' : 'opacity-0'}`}>
          <p className="font-body text-base text-center" style={{ color: '#C43535' }}>
            <strong style={{ color: '#A72828' }}>Trusted by 15,247+ women</strong> across South Africa
            <br />
