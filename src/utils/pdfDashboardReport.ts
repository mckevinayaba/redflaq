...
const PURPLE = [107, 33, 168] as const; // #6B21A8
const DARK = [59, 7, 100] as const; // #3B0764
...
export function generateDashboardReport(data: ReportData) {
  ...
  doc.setFontSize(9);
  doc.text("Dashboard Report", margin + 16 + rfW + doc.getTextWidth("q") + 4, y + 9);
  ...
  // --- Section: Daily Checks Chart (simple bar representation) ---
  doc.setTextColor(...DARK);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(14);
  ...
  const barW = contentW / data.dailyChecks.length;
  ...
  data.dailyChecks.forEach((day, i) => {
    const barH = (day.count / maxCount) * (chartH - 2);
    const bx = margin + i * barW;
    const by = y + chartH - barH;

    if (day.count > 0) {
      doc.setFillColor(...PURPLE);
      doc.rect(bx + 0.5, by, barW - 1, barH, "F");
    }
  ...
  // Save
  const fileName = `redflaq-dashboard-report-${today.toISOString().split("T")[0]}.pdf`;
  doc.save(fileName);
}
